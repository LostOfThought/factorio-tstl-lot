...
function _formatStructuredArg(arg: any, seen: Set<any>): string {
    ...
    if (typeof arg === 'object') { // This check should be after specific known object types
        ...
        if (Array.isArray(arg)) {
            ...
            const objectArg = arg as { [key: string]: any }; // Linter fix: Assert arg is an indexable object
...
export default {
    log: (message?: any, ...optionalParams: any[]): void => {
        ...
        if (typeof args[0] === 'string') {
            ...
            const formattedString = fmtStr.replace(formatSpecifierRegex, (match) => {
                if (currentArgIndex < substitutionArgs.length) {
                    ...
                    switch (match) {
                        ...
                        case '%d':
                        case '%i':
                            const num = parseInt(String(currentArg), 10);
                            return Number.isNaN(num) ? 'NaN' : String(num);
                        case '%f':
                            const floatNum = parseFloat(String(currentArg));
                        ...
                        default:
                            return match; // Should not be reached with the current regex
                    }
                }
